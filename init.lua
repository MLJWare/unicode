...
function unicode.sub(text, i, j)
  ...
  local i2 = utf8.offset(text, (i-1 > text_len) and text_len + 1 or i) or 1

  if not j then return raw_sub(text, i2) end

  local j2 = (utf8.offset(text, j + 1) or 0) - 1
...
function unicode.upper(text)
  ...
  local result = {}
  for _, cc in utf8.codes(text) do
    insert(result, char(upper[cc] or cc))
  end
  return table.concat(result)
end
...
function unicode.lower(text)
  ...
  local result = {}
  for _, cc in utf8.codes(text) do
    insert(result, char(lower[cc] or cc))
  end
  return table.concat(result)
end
